...
- Defaults to Auto frame rate and Auto bitrate, so it does not force custom FPS or bitrate unless you select one.
...
Exports happen in real time. A 60-second clip usually takes about 60 seconds to render. By default, frame rate and bitrate are left to the browser/source timing; use the controls only when you want to force a specific value.

## Suggested prompt used to build this

Build a static GitHub Pages app with no backend. The app should let a user select a local video file, preview it on a canvas, choose short-video social formats such as 9:16 YouTube Shorts/Reels/TikTok, square 1:1, and 16:9 landscape, choose crop or contain fitting, optionally add a blurred background, trim start and end time, and export the result in the browser using MediaRecorder. Frame rate and bitrate should default to Auto so the app does not force a custom FPS or bitrate unless the user explicitly selects one. Use plain HTML, CSS, and JavaScript so the user can unzip the files, upload them to a GitHub repository, enable GitHub Pages, and run the app. Include polished responsive UI, privacy notes, export progress, thumbnail export, and clear deployment instructions.
